...
class OneUpHealthBearerTokenNotAddedToOtherFhirProvidersGatewaySpec extends AbstractFhirEHRGatewayWithWiremockSpec {

	def "Get gender from fhir through something else than 1up.health service"() {
		given: "A female patient 'Susanne'"
		def patientCDWReference = patientSusanneNotOneUp()
		...
		and: "No Authorization header is used"
		verify(getRequestedFor(urlEqualTo("/baseDstu2/Patient/" + PATIENT_SUSANNE_RESOURCE_ID))
	...
	PatientCDWReference patientSusanneNotOneUp() {
		stubFor(get(urlEqualTo("/baseDstu2/Patient/" + PATIENT_SUSANNE_RESOURCE_ID))

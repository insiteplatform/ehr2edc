...
import com.custodix.insite.local.ehr2edc.security.oidc.EHR2EDCOpenIDConnectFilter
import com.custodix.insite.local.ehr2edc.security.oidc.EHR2EDCRsaKeyResolver
import com.custodix.insite.local.ehr2edc.security.oidc.EHR2EDCUserAuthenticationFactory
...
class EHR2EDCOpenIDConnectFilterSpec extends Specification {
    ...
    private EHR2EDCOpenIDConnectFilter openIDConnectFilter

    void setup() {
        getUser = mock(GetUser)
        def user = GetUser.User.newBuilder().withDrm(true).withUserId(UserIdentifier.of("test")).withName("username").build()
        when(getUser.getUser(Mockito.any())).thenReturn(GetUser.Response.newBuilder().withUser(user).build())

        restTemplate = mock(OAuth2RestTemplate)
        EHR2EDCRsaKeyResolver rsaKeyResolver = new EHR2EDCRsaKeyResolver(OBJECTMAPPER, KEYSTORE);
        def userAuthFactory = new EHR2EDCUserAuthenticationFactory(getUser)
        openIDConnectFilter = new EHR2EDCOpenIDConnectFilter(DEFAULT_URL, restTemplate, userAuthFactory, rsaKeyResolver)
    }

    def "Validating a correct token"() {
        given: "A valid encryption key in the keystore under kid 'rsa1'"

...
class EHR2EDCOpenIDConnectFilter extends AbstractAuthenticationProcessingFilter {

	private static final Logger LOGGER = LoggerFactory.getLogger(EHR2EDCOpenIDConnectFilter.class);

	private final OAuth2RestOperations restTemplate;
	private final EHR2EDCRsaKeyResolver ehr2edcRsaKeyResolver;
	private final EHR2EDCUserAuthenticationFactory ehr2edcUserAuthenticationFactory;

	EHR2EDCOpenIDConnectFilter(String defaultFilterProcessesUrl, OAuth2RestTemplate restTemplate,
			EHR2EDCUserAuthenticationFactory ehr2edcUserAuthenticationFactory, EHR2EDCRsaKeyResolver ehr2edcRsaKeyResolver) {
		super(defaultFilterProcessesUrl);
		this.restTemplate = restTemplate;
		this.ehr2edcRsaKeyResolver = ehr2edcRsaKeyResolver;
		this.ehr2edcUserAuthenticationFactory = ehr2edcUserAuthenticationFactory;
	}
	...
	private RSAKey getRsaKey(SignedJWT signedJWT) {
		String keyId = signedJWT.getHeader()
				.getKeyID();
		return ehr2edcRsaKeyResolver.resolve(keyId);
	}
	...
	private Authentication getAuthenticationToken(OAuth2AccessToken accessToken) throws ParseException {
		...
		return ehr2edcUserAuthenticationFactory.create(webToken);
	}

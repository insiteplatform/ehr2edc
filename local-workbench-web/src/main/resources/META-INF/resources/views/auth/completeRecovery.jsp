...
<body>
					...
					<c:choose>
						<c:when test="${requestScope.expired}">
							<p class="alert-box info">
								<i class="fa fa-exclamation-circle"></i> Your account recovery request has expired. Please issue a new account recovery request.
							</p>
						</c:when>
						...
						<c:otherwise>
							...
							<p class="margin-bottom">Please enter a new password to complete account recovery.</p>
							...
							<div class="grid-content collapse shrink">
								<a id="recoverAccount" class="button accent">Recover Account</a>
							</div>
							<p class="alert-box accent hide recover-success">
								<i class="fa fa-info"></i> Your account has been recovered. You can now <a href="${loginURL}">log in</a> with your new password.
							</p>
							<div class="alert-box warning hide recover-error" style="white-space: pre-wrap;"></div>
						</c:otherwise>
					</c:choose>
				</article>
			</section>
	...
	<script type="text/javascript" src="<c:url value="/assets/auth/js/completeRecovery.js"/>"></script>
